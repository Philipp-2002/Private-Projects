...
def place(board, slot, symbol): #place one piece in the slot
    ...
    for tiefe in range(6): #gehe von oben nach unten in den slot
        if board[slot][tiefe] is not None:  #das neue Teil stoesst auf ein anderes
            board[slot][tiefe-1] = symbol#platziere das neue Teil darüber
            return board
        
    board[slot][5] = symbol #wenn der ganze Slot frei ist, kann das Teil ganz unten platziert werden
    return board
        ...
        for ev in events:
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                if pos[0] > 0 and pos[0] < 700 and pos[1] > 100 and pos[1] < 600:
                    slot = int(pos[0]/100)
                    newBoard = place(board, slot, "O") #player platziert "O"
                    print(slot)
                    return newBoard



def minimax(board, isMaximizing, alpha, beta, depth): #helper function for makeBestMove()
    ...
    if isMaximizing:
        bestScore = -10000000
        for slot in slotOrder: #permutation of all slots
            if board[slot][0] is None:
                newBoard = deepcopy(board)
                newBoard = place(newBoard, slot, "X")
                score = minimax(newBoard, False, alpha, beta, depth + 1)
        ...
        for slot in slotOrder:
            if board[slot][0] is None:
                newBoard = deepcopy(board)
                newBoard = place(newBoard, slot, "O")
                score = minimax(newBoard, True, alpha, beta, depth + 1)
...
def makeBestMove(board, isMaximizing): #program calculates move
    ...
    for slot in slotOrder:
        if board[slot][0] is None:
            newBoard = deepcopy(board)
            newBoard = place(newBoard,slot,"X" if isMaximizing else "O")
            score = minimax(newBoard, not isMaximizing, -1000000, 1000000,0)
    ...
    board = place(board, bestSlot, "X" if isMaximizing else "O")
    return board
