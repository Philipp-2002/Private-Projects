    ...
    for slot in range(4): #die 3 rechtesten slots fallen weg
        for tiefe in range(6):
            if board[slot][tiefe] == "X" and board[slot+1][tiefe] == "X" and board[slot+2][tiefe] == "X" and board[slot+3][tiefe] == "X":
                return (1000, [[slot,tiefe],[slot+1,tiefe],[slot+2,tiefe],[slot+3,tiefe]])
    ...
    for slot in range(7):
        for tiefe in range(3,6): #die 3 obersten tiefen fallen weg
            if board[slot][tiefe] == "X" and board[slot][tiefe-1] == "X" and board[slot][tiefe-2] == "X" and board[slot][tiefe-3] == "X":
                return (1000,[[slot,tiefe],[slot,tiefe-1],[slot,tiefe-2],[slot,tiefe-3]])
    ...
    for slot in range(4):   #diagonalen nach rechts oben
        for tiefe in range(3,6):
            if board[slot][tiefe] == "X" and board[slot+1][tiefe-1] == "X" and board[slot+2][tiefe-2] == "X" and board[slot+3][tiefe-3] == "X":
                return (1000,[[slot,tiefe], [slot+1,tiefe-1], [slot+2,tiefe-2], [slot+3,tiefe-3]])
...
def heuristicEval(board):
    ...
    for slot in range(0, 4):
        for tiefe in range(6):
            if board[slot][tiefe] == "X" and board[slot+1][tiefe] == "X" and board[slot+2][tiefe] == "X":
                if slot > 0:
                    if board[slot-1][tiefe] == None:
                        score += 10
                if board[slot+3][tiefe] == None:
                    score += 10
                        
            if board[slot][tiefe] == "O" and board[slot+1][tiefe] == "O" and board[slot+2][tiefe] == "O":
                if slot > 0:
                    if board[slot-1][tiefe] == None:
                        score -= 10
                if board[slot+3][tiefe] == None:
        ...
        for ev in events:
            if ev.type == pygame.MOUSEBUTTONDOWN:
                ...
                if pos[0] > 0 and pos[0] < 700 and pos[1] > 100 and pos[1] < 600:
                    slot = int(pos[0]/100)
                    if board[slot][0] is None:  #wenn noch platz in dem slot ist
